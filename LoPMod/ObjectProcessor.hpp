...
void DrawBlockingVolumeWireframe(ABlockingVolume* BlockingVolume, AHUD* HUD, FLinearColor LineColor, float LineThickness)
{
    if (!BlockingVolume || !HUD)
    {
    ...
    if (DistanceToPlayer <= distanceToDraw)
    {
        UBodySetup* BrushBodySetup = BlockingVolume->BrushComponent->BrushBodySetup;
...
void blockingVolumeProcessor(ABlockingVolume* blockingVolume, AHUD* HUD, UFont* font) {
    if (showBlocking) {
        ...
        if (DistanceToPlayer <= distanceToDraw)
            DrawBlockingVolumeWireframe(blockingVolume, HUD, BLUE, 1.0f);
    }
